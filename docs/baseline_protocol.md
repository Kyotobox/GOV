...
- **`TelemetryService`**: (Indirecto) Su data es considerada en la auditoría previa.
...
2.  **Aprobación del Product Owner (PO)**:
    ...
    -   El `VanguardCore` emite un desafío de nivel `STRATEGIC-GOLD`, solicitando la firma del PO para autorizar el sellado del kernel.
    -   Este paso es un "Human-in-the-Loop" (HITL) obligatorio. El sistema espera una firma RSA válida del PO dentro de un tiempo límite (60 segundos). Si la firma no se recibe o es inválida, el `baseline` se aborta.

3.  **Resolución de Clave Privada del PO**:
...
7.  **Auto-Commit Semántico (TASK-S16-01)**:
    -   Se realiza un `git add .` y un `git commit` de forma automática.
    -   El mensaje del commit se genera de forma semántica, incluyendo el ID del sprint y, si hay una tarea activa, su ID y descripción (ej. `feat(S15-SYMBIOSIS): TASK-DPI-S15-01 - Generador de Contexto`).
...
-   **Aprobación HITL (Human-in-the-Loop)**: La exigencia de una firma RSA del PO para sellos `STRATEGIC-GOLD` introduce una capa de seguridad humana y deliberada, previniendo `baselines` no autorizados o accidentales.
...
-   **Auto-Commit Semántico**: Asegura que cada `baseline` esté explícitamente registrado en el historial de control de versiones con un mensaje descriptivo, mejorando la trazabilidad y la auditoría del código.
